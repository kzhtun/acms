    ...
    <li className="gx-media">
      ...
      <div className="gx-media-body">
        <div className="gx-flex-row gx-justify-content-between gx-align-items-center">
          <h5 className="gx-text-capitalize gx-user-name gx-mb-0"><a href="javascript:void(0);">{name}</a></h5>
          <span className="gx-meta-date"><small>{time}</small></span>
        </div>
        <p className="gx-fs-sm">{message}</p>
        <span className="gx-btn gx-btn-xs gx-text-muted"><i className="icon icon-reply gx-pr-2"/>Reply</span>
        <span className="gx-btn gx-btn-xs gx-text-muted"><i className="icon icon-custom-view gx-pr-2"/>Read</span>
      </div>
    </li>
  );
};
